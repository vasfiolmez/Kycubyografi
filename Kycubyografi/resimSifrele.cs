    ...
    public partial class resimSifrele : Form
    {
        ...
        private void btnSifreliDosyasec_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    type[j] = Encoding.ASCII.GetString(BitConverter.GetBytes((short)detype));
        ...
        private void btnSifreCoz_Click(object sender, EventArgs e)
        {
            ...
            for (int j = 3; j < 16; j++)
            {
                ...
                tlength += Convert.ToInt32(Encoding.ASCII.GetString(BitConverter.GetBytes((short)delength)));
            }
            ...
            for (int i = 0; i < EncryptedImage.Height; i++)
            {
                for (int j = 0; j < EncryptedImage.Width; j++)
                {
                    if (k < length)
                    {
                        ...
                        txtGizliYazı.Text += Encoding.ASCII.GetString(BitConverter.GetBytes((short)demsg));
